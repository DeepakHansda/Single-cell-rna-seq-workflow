...
**explanation**
`rowData(sce.zeisel)$Ensembl` will add one more column to the `rowData` field. `mapIDs()` function work like following,
...
keytype="the type of gene names (`key`) found in annotation object", column="The column we want to extract fromthe annotation object")
```
